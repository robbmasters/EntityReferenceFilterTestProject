...
class Tracker implements TrackerInterface {
   ...
   *   The path validator.
   */
  public function __construct(ConfigFactory $config_factory, Connection $connection, EntityRepositoryInterface $entity_repository, EntityTypeManagerInterface $entity_type_manager, MenuLinkManagerInterface $menu_link_manager, ModuleExtensionList $module_extension_list, InboundPathProcessorInterface $path_processor, RequestStack $request_stack, RequestMatcherInterface $router, PathValidatorInterface $path_validator) {
  ...
  public function getTrackedMenus() {
    if ($this->config->get('all_menus')) {
      return array_keys($this->getAllMenus(FALSE));
    }
  ...
  protected function setTrackedMenus(array $menus = [], $force = FALSE) {
    $old_values = array_combine($this->getTrackedMenus(), $this->getTrackedMenus());

    // Preserve tracking for inaccessible menus.
    $inaccessible_menus = array_diff_key($this->getAllMenus(FALSE), $this->getAllMenus(TRUE));
    $inaccessible_menus = array_intersect_key($old_values, $inaccessible_menus);
    $menus = array_merge($menus, $inaccessible_menus);
    asort($menus);
    ...
    if ($force) {
      ...
      $this->untrackMenus(array_keys($this->getAllMenus(FALSE)));
      $this->trackMenus($this->getTrackedMenus());
    }
    else {
  ...
  protected function addEntityTargets(EntityInterface $entity, array $targets = []) {
    ...
    $host_values = [
      ...
      'parent_uuid' => $parent_link ? (string) $parent_link->getDerivativeId() : '',
